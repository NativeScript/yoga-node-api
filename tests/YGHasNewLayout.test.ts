...
Deno.test("new_layout_can_be_marked_seen", () => {
  const root = Yoga.Node.create();
  root.markLayoutSeen();
  expect(root.hasNewLayout()).toBe(false);
});

Deno.test("new_layout_calculating_layout_marks_layout_as_unseen", () => {
  const root = Yoga.Node.create();
  root.markLayoutSeen();
...
Deno.test("new_layout_calculated_layout_can_be_marked_seen", () => {
  const root = Yoga.Node.create();
  root.calculateLayout(undefined, undefined);
  root.markLayoutSeen();
  expect(root.hasNewLayout()).toBe(false);
...
Deno.test("new_layout_recalculating_layout_does_mark_as_unseen", () => {
  const root = Yoga.Node.create();
  root.calculateLayout(undefined, undefined);
  root.markLayoutSeen();
  root.calculateLayout(undefined, undefined);
...
Deno.test("new_layout_reset_also_resets_layout_seen", () => {
  const root = Yoga.Node.create();
  root.markLayoutSeen();
  root.reset();
  expect(root.hasNewLayout()).toBe(true);
...
Deno.test("new_layout_children_sets_new_layout", () => {
  const root = Yoga.Node.create();
